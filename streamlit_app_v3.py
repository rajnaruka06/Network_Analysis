...
def generate_report(network_statistics, ergm_results, alaam_results):
    ...
    <ul>
        <li>Number of Nodes: {num_nodes}</li>
        <li>Number of Edges: {num_edges}</li>
        <!-- Include other network statistics here -->
    </ul>
    
    <h2>ERGM Analysis Results</h2>
    <p>{ergm_results}</p>
    
    <h2>ALAAM Analysis Results</h2>
    <p>{alaam_results}</p>
    """.format(
        num_nodes=network_statistics['Number of Nodes'],
        num_edges=network_statistics['Number of Edges'],
        ergm_results=ergm_results,
        alaam_results=alaam_results
    )
    return report_content

def get_binary_file_downloader_html(binary_file, file_name, button_text="Download Report File"):
...
if __name__ == "__main__":
    ...
    if uploaded_file is not None:
        ...
        if report_button:
        # Generate the report content
         report_content = generate_report(network_statistics)
        
        # Set up download link
         st.markdown(report_content, unsafe_allow_html=True)
        
        # Trigger download
         st.markdown(get_binary_file_downloader_html(report_content, file_name="network_analysis_report.html"), unsafe_allow_html=True)

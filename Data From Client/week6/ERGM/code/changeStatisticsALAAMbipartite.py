...
def changeBipartiteoO(attrname, mode, G, A, i):
    """change statistic for binary exogenous attribute oO (outcome
    attribute related to binary attribute on same node)

    [*]
    """
    return 0 if (G.binattr[attrname][i] == NA_VALUE or G.bipartite_node_mode(i) != mode) else G.binattr[attrname][i]
   
def changeBipartite_A_b(attrname, mode, G, A, i):
    """change statistic for continuous exogenous attribute A_bB (outcome
    attribute related to continuous attribute on attribute of connected node from the other mode)

    *--[o]
    """
    return 0 if (G.bipartite_node_mode(i) != mode) else sum([G.contattr[attrname][j] for j in G.neighbourIterator(i)])


def changeBipartite_A_bB(attrnameA, attrnameB, mode, G, A, i):
    """change statistic for binary exogenous attribute A_bB (outcome
    attribute related to binary attribute on same node and 
    attribute of connected node from the other mode)

    [*]--[o]
    """
    return 0 if (G.binattr[attrnameB][i] == NA_VALUE or G.binattr[attrnameB][i] == 0 or G.bipartite_node_mode(i) != mode) else sum([G.binattr[attrnameA][j] for j in G.neighbourIterator(i)])
